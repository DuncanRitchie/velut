...
class Subwords extends Component {
    ...
    render() {
        let input = window.location.pathname.replace("/Subwords","").replace("/","")
        document.title = "Subwords of "+input+" on velut"

        let mappedWords = []
        if (this.state.allWords.length) {
            if (this.state.allWords[0].Word) {
                let sortedWordObjects = findSubwords(input,this.state.allWords)
        ...
        return (
            <div className="word">
                <h1><span className="title">velut</span> &mdash; Subwords &mdash; {input}</h1>
