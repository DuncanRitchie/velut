...
const getScansionDescriptionRecursive = (scansion) => {
  ...
  const head = getScansionDescriptionRecursive(
    scansion.substring(0, headLength),
  )
...
const getScansionDescription = (scansion) => {
  return (
    getFootNameInData(scansion)?.replace(',', ' or') ??
    getScansionDescriptionRecursive(scansion)
  )
}
