...
import { splitIntoWords } from './multiHelpers'
...
export default async function findManyWords(searchWordsAsString) {
  ...
  return await Promise.allSettled(promises).then((results) => {
    const allDistinctWordObjects = results.map((result) => {
    ...
    const allWordObjects = searchWords.map((searchWord) => {
      ...
      return {
        ...allDistinctWordObjects.find((wordObject) => wordObject.search === searchWord),
      }
    })
