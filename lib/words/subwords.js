...
export const findSubwordsInWordsFromMongo = (input, wordObjects) => {
    ...
    console.log(`findSubwordsInWordsFromMongo called with input ${input}`)
    const regex = generateRegexForSubwords(input)
    const filteredWordObjects = wordObjects.filter(word => {
        // console.log({word: word.Word, test: regex.test(word.AlphOrderNoMacra)});
        return regex.test(word.AlphOrderNoMacra)
    })
    // console.log({filteredWordObjects})
...
export function deleteCharactersOptimised(superwordAlphOrder, subwordAlphOrder) {
    // console.log({superwordAlphOrder, subwordAlphOrder})
    ...
    while (indexInSuperword < superwordAlphOrder.length
        && indexInSubword < subwordAlphOrder.length) {
            ...
            // console.log({currentSuperwordChar, currentSubwordChar})

            if (currentSuperwordChar > currentSubwordChar) {
                // console.log(`Letter ${currentSubwordChar} is not in ${superwordAlphOrder} which we know because it’s earlier than ${currentSuperwordChar} — deletion failed for ${subwordAlphOrder}!`)
                return null;
            }
            if (currentSuperwordChar === currentSubwordChar) {
                // console.log(`Match of ${currentSubwordChar} at superword ${indexInSuperword} subword ${indexInSubword}`)
                indexInSubword++;
            }
            else {
                // console.log(`No match of ${currentSuperwordChar} at superword ${indexInSuperword} ${currentSubwordChar} subword ${indexInSubword}`)
    ...
    if (indexInSuperword === superwordAlphOrder.length
        && indexInSubword < subwordAlphOrder.length) {
        // console.log('Went through letters of superword before all letters of subword — deletion failed!')
        return null
    }
    // If we have gone through all the letters in subword and there are letters left in superword.
    while (indexInSuperword < superwordAlphOrder.length) {
        // console.log(`Adding ${superwordChars[indexInSuperword]}`)
        remainingChars += superwordChars[indexInSuperword];
        indexInSuperword++;
    }

    // console.log(`After running function, remainingChars is “${remainingChars}”`)
