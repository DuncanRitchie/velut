...
const log = logger.child({ module: 'word' })
...
const regexWMeaningVUpperCase = RegExp(`(?<!${anyConsonant})W(?!${anyConsonant})(?=.)`, 'g')
const regexWMeaningVLowerCase = RegExp(`(?<!${anyConsonant})w(?!${anyConsonant})(?=.)`, 'g')
...
const findOneWord = async function (searchWord, selection) {
  ...
  const executeQuery = async (indexOfQuery) => {
    try {
      const foundWord = await Word.findOne(funcsReturningQueries[indexOfQuery](normalisedWord)).select(selection).exec()
