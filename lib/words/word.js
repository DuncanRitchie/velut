...
const findOneWord = async function (searchWord, selection) {
  ...
  const normalisedWord = searchWord
    ...
    //// The regex is case-insensitive because the lookarounds need to be.
    .replace(/(?<![bcdfghklmnpqrstvwxz])w(?![bcdfghklmnpqrstvwxz])(?=.)/gi, 'v')
    //// w is interpreted as u in other positions (next to a consonant or at word-end).
    .replaceAll('W', 'U')
    .replaceAll('w', 'u')
