// This file (subwordsHelpers) is safe to import on the front-end because it does not use Mongoose.
...
export function deleteCharactersOptimised(superwordAlphOrder, subwordAlphOrder) {
  ...
  while (indexInSuperword < superwordAlphOrder.length && indexInSubword < subwordAlphOrder.length) {
  ...
  if (indexInSuperword === superwordAlphOrder.length && indexInSubword < subwordAlphOrder.length) {
    return null
  }
...
export function generateRegexForSubwords(superword) {
  const lettersOnly = superword.replace(/[^a-z]/gi, '').toLowerCase()
  const lettersInAlphOrder = sortStringAlphabetically(lettersOnly)
  let pattern = '^'
  for (let i = 0; i < lettersInAlphOrder.length; i++) {
    pattern += lettersInAlphOrder[i]
    pattern += '?'
  }
  pattern += '$'
