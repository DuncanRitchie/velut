...
async function getCognates(propsContainingRoot) {
  try {
    ...
    // This code for finding cognates considers all the roots in Roots, or simply Root (if there is no Roots).
    // My intention over time is that I will fill out all lemmata with a Roots field, and then I’ll deprecate Root.
    const roots = propsContainingRoot.Roots || [propsContainingRoot.Root]
