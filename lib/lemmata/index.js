...
export function getShowGeneratedFormsString() {
  ...
  return listFormatter.format(showGeneratedFormsFor)
}
...
export async function getLemmata(
  query,
  sort,
  limit = Infinity,
  getFormsAndCognates = true,
) {
  try {
    await dbConnect()
    const selection = {
    ...
    if (foundLemmata && foundLemmata.length) {
      ...
      if (getFormsAndCognates) {
        await Promise.all(
          mappedLemmata.map(async (lemma) => {
            ...
            lemma.incorrectForms = incorrectForms
          }),
        )
      }
      ...
      const limitedLemmata =
        limit !== Infinity ? mappedLemmata.slice(0, limit) : mappedLemmata
