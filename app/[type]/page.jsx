...
export async function getMetadata({ params }) {
  const { foundWord, headingToDisplay } = await getData(params)
  if (!foundWord || !headingToDisplay) {
    return {}
  }
  const metadata = {
    title: `“${foundWord.Word}” on velut — a Latin rhyming dictionary`,
    description: `${headingToDisplay} for “${foundWord.Word}”, also showing its meaning, forms, cognates, and links to other dictionaries.`,
  }
  console.table(metadata)
  return metadata
}

async function WordPage({ params }) {
  let {
    sanitisedInput,
    foundWord,
    homographs,
              ...
              <>
                {incorrectLemmata.length ? (
                  <p>
                    ...
                    {mappedIncorrectLemmata}, but this is probably wrong. I will
                    remove it if it’s unacceptable Latin!
                  </p>
                ) : (
                  <p>
...
async function getData(props) {
  ...
  if (!params) {
    throw Error('Hello you need to define props.params')
  }
  ...
  //// If the word was not found, we trigger the not-found page to be shown instead.
  else {
    // Hack to work around a bug in Next.js
    // Next.js doesn’t pass route segments into not-found pages like it does for normal pages,
    // so we can’t just pull wordParam & typeParam from the route in not-found.jsx.
    // Instead, let’s mutate Node’s `global` object and stick the params there!
    // https://github.com/vercel/next.js/discussions/43179#discussioncomment-9738369
    if (wordParam !== 'favicon.ico') {
      global.hackForNotFoundParams = {
        wordParam,
        typeParam,
        sanitisedInput,
      }
